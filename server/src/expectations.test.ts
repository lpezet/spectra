...
// Same dance as raise.test.ts: store.ts resolves SPECS_DIR at module load, so the sandbox has
// to exist and be pointed at before anything imports it.
let specs: string
let raiseExpectation: typeof import('./expectations.js').raiseExpectation
let supersedeExpectation: typeof import('./expectations.js').supersedeExpectation
let readExpectations: typeof import('./store.js').readExpectations
...
beforeAll(async () => {
  ...
  ;({ raiseExpectation, supersedeExpectation } = await import('./expectations.js'))
  ;({ readExpectations } = await import('./store.js'))
})
...
describe('raiseExpectation', () => {
  it('writes a live expectation with no review step', async () => {
    const outcome = await raiseExpectation(BASE)
  ...
  it('numbers the next above the highest already there', async () => {
    const second = await raiseExpectation({ ...BASE, expect: 'a second thing holds' })
  ...
  it('refuses a functional expectation naming no terms, since it could never be counted', async () => {
    ...
    const outcome = await raiseExpectation({ ...BASE, terms: [], expect: 'something vague' })
...
describe('supersedeExpectation', () => {
  it('moves the original to retired/, records why, and points at the replacement', async () => {
    const original = await raiseExpectation({ ...BASE, expect: 'the first wording' })
    expect(original.ok).toBe(true)
    if (!original.ok) return

    const outcome = await supersedeExpectation(original.id, {
    ...
    const { expectations, retired } = await readExpectations()
  ...
  it('retires without a replacement, and the directory is what marks it retired', async () => {
    const original = await raiseExpectation({ ...BASE, expect: 'this turned out to be wrong' })
    expect(original.ok).toBe(true)
    if (!original.ok) return

    const outcome = await supersedeExpectation(original.id, { note: 'the feature was dropped' })
  ...
  it('never reuses a retired id', async () => {
    const before = await raiseExpectation({ ...BASE, expect: 'about to be retired' })
    expect(before.ok).toBe(true)
    if (!before.ok) return

    await supersedeExpectation(before.id, { note: 'no longer relevant' })
    const after = await raiseExpectation({ ...BASE, expect: 'raised after the retirement' })
  ...
  it('404s on an id that is not live', async () => {
    const outcome = await supersedeExpectation('e-999', { note: 'nothing there' })
    expect(outcome.ok).toBe(false)
    if (outcome.ok) return
