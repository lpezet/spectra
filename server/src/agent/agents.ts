...
6. Call mark_implemented with the changeset id. It is refused unless app/specs.snapshot.json is at the current specs version — the same way a push is refused when the remote has moved. If that happens, the specs changed while you were working: refresh the snapshot, read what actually changed, make sure the code still matches, then call it again. There is no override, and asking for one is not the answer.
