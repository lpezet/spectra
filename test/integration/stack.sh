#!/usr/bin/env bash
#
# Full-stack e2e: build the images, boot server + web via compose, and assert the running stack
# actually serves — the web UI, and /api proxied through nginx to server. Tears down on exit.
#
...
echo "  building + starting server + web…"
export ANTHROPIC_API_KEY="" CLAUDE_CODE_OAUTH_TOKEN=""
"${COMPOSE[@]}" up -d --build server web >/dev/null 2>&1 || fail "compose up" "the stack did not start"
...
curl -s --retry 60 --retry-all-errors --retry-connrefused --retry-delay 1 -o /dev/null localhost:5174/api/project \
  || fail "server readiness" "no /api after ~60s"
pass "stack is up (server + web built and started)"

# the server serves the API directly.
[ "$(code localhost:5174/api/project)" = 200 ] || fail "server /api/project" "not HTTP 200"
curl -s localhost:5174/api/project | grep -q '"name"' || fail "server identity" "no project name in /api/project"
pass "server serves /api directly"
...
# The load-bearing assertion: /api proxied through nginx to the server on the compose network.
curl -s localhost:5173/api/project | grep -q '"name"' || fail "nginx proxy" "/api not proxied to the server"
...
pass "web proxies /api to the server ($TERMS terms via nginx)"
