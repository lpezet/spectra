...
describe('LocalAuthorizer', () => {
  ...
  it('stamps a bare human, with no account', async () => {
  ...
  it('offers the one configured org', async () => {
    expect((await authorizer.authenticate()).orgs()).toEqual(['local'])
  })

  it('allows every org and project', async () => {
    const principal = await authorizer.authenticate()
...
describe('resolveAuthorizer', () => {
  ...
  it('defaults to the local authorizer, offering the configured org', async () => {
    ...
    expect((await authorizer.authenticate({} as never)).orgs()).toEqual(['acme'])
  })
  ...
  it('loads a plugin authorizer and hands it the context', async () => {
    ...
    expect((await authorizer.authenticate({} as never)).orgs()).toEqual(['from-plugin:acme'])
  })
